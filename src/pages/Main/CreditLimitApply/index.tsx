...
const statusStyles = {
  ...
  refused_by_customer: 'bg-red-100 text-red-800 hover:bg-red-200',
  ...
  'to approve': 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200',
  'to confirm': 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200',
};
...
const CreditLimitApplyRow = ({ item }) => {
  return (
...
const CreditLimitApply = () => {
  ...
  useEffect(() => {
    const fetchCreditLimitsApply = async () => {
      ...
      const onError = () => {
        setLoading(false);
        toast.error('Failed to fetch credit limit applications');
      };
      setLoading(true);
  ...
  const filteredCreditLimits = creditLimitsApply.filter((item) => {
    const matchesSearch =
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg">
                  {searchQuery
                    ? 'No credit limit applications found matching your search.'
                    : 'No credit limit applications found.'}
                </p>
                <p className="text-gray-400 text-sm mt-2">
                  {!searchQuery &&
                    'Add your first credit limit application to get started.'}
                </p>
              </div>
            ) : (
              <>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredCreditLimits.map((item) => (
                        <CreditLimitApplyRow key={item.id} item={item} />
                      ))}
                    </TableBody>
                  </Table>
                </div>
                ...
                <div className="md:hidden space-y-4">
                  {filteredCreditLimits.map((item) => (
                    <CreditLimitApplyCard key={item.id} item={item} />
                  ))}
                </div>
              </>
            )}
